...
static void runExploit(void) {
    ...
    // Elevate to root (ucred swap with launchd) — needed so the HUD child
    // inherits a platform/root credential and SpringBoard hosting accepts us.
    int eret = sandbox_elevate_to_root(self_proc_addr);
    NSLog(@"[Tweak] sandbox_elevate_to_root returned %d (uid=%d)", eret, getuid());

    // Platformize our AMFI slot too (belt & suspenders for mach lookups).
    int pret = platformize_self(self_proc_addr);
    NSLog(@"[Tweak] platformize_self returned %d", pret);
