...
//  Reworked to match cyanide's statbar pattern (verified working):
//    - UIWindow created with initWithWindowScene: (iOS 13+ REQUIREMENT —
//      initWithFrame: windows without a scene never appear on screen)
//    - Content is a UILabel added via addSubview: + setText: (the old
//      version created an empty window with NO subviews → invisible)
//    - Window retained via objc_setAssociatedObject on UIApplication so
//      ARC/destroy_remote_call cannot release it
...
// Update the overlay text from the local process (statbar setText pattern).
