...
uint64_t hide_path(const char* path) {
    ...
    if(vnode == -1) {
        TweakLog("[%s:%d] Unable to get vnode, path: %s", __FUNCTION__, __LINE__, path);
        return -1;
    }
...
uint64_t overwrite_system_file(char* to, char* from) {
    ...
    if(to_file_sz < from_file_sz) {
        close(from_fd);
        close(to_fd);
        TweakLog("[%s:%d] File size is too big to overwrite!", __FUNCTION__, __LINE__);
        return -1;
    }
    ...
    uint64_t to_fileproc = xpaci(kread64(fileprocPtrArr + (8 * to_fd)));
